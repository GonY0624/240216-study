...
public class Application {
    ...
    public static void main(String[] args) {
        ...
        /* 수업목표. 객체지향 프로그래밍에 대해 이해 하고 객체와 클래스를 성계하여 프로그래밍 할 수 있다.  */
        /* 필기.
        *       객체 지향 프로그래밍이란?
        *   현실세계의 모든 사건 (event )는 객체와 객체의 상호작용에 의해 일어난다는 세계관을
        *   프로그램을 만들 떄 이용하여 새로운 세계를 창조하는 방법론이다.
        *   현실 세계와 닮아있지만, 다른 점은
        *   객체간의 상호작용을 한다는 사실은 현실 세계와 동일 하지만 의잉ㄴ화 기법이 적용되어
        *   현실세계에서 불가능한 무생물이나 개념같은 존재들도 하나의 주체로
        *   본인의 상태를 스스로 제어하고 행동하도록 한다는 것이 현실세계와 다른 점이다. (캡슐화)
        ...
        *  */
        ...
        /* 필기.
        *   객체를 설계하기 위해서는 복잡한 현실세계를 반영하기에는 너무 방대하고 복잡하기 떄문에
        *   현실 세계를 프로그램의 목적에 맞게 단순화하는 '추상화' 라는 기법을 적용하게 된다.
        *  */

        /* 필기.
        *   추상화란?
        *   공통된 부분의 추출하고 공통되지 않은 부분을 제거한다는 의미를 가지며, 추상화의 목적은
        *   유연성을 확보하기 위함이다.
        *   유연성 확보는 여러 곳에 적용될 수 있는 유연한 객체를 의미하며, 즉 재상용성이 높아질 수
        *   있게 한다는 의미이다.
        ...
        *   5. 카레이서는 엑셀레이터를 밟으면 시동이 걸렸다면 자동차는 시속이 10 km/h 증가하며 앞으로 나간다.
        *   6. 자동차는 달리는 중인 경우 브레이크를 밟으면 자동차의 시속은 0으로 떨어지며 멈춘다.
        *   7. 브레이크를 밟을 떄 자동차가 달리는 중이 아니라면 이미 멈처있는 상태라고 안내한다.
        ...
        *    2. 자동차 클래스
        *    - 속성: 시동 상태, 현재 시속
        *    - 행위: 시동을 가라, 앞으로 가라, 멈춰라, 시동을 꺼라
        *  */

        /* 필기. 카레이서가 한 명 등장합니다.  */
        CarRacer racer = new CarRacer();

        /* 필기. 입력받은 메뉴에 따라 카레이서의 행동이 달라집니다.  */
