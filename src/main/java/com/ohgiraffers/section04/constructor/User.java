...
public class User {
    ...
    /* 필기.
    *   생성자의 작성 위치
    *   문법상으로 클래스 내부에 작성하명 되지만,
    *   통상적으로 필드 선언부와 메소드 선언부 사이에 적는 것이 관례이다.
    *  */

    /* 필기.
    *   생성자의 사용 목적
    *   1. 이스턴스 생성 시점에 수행할 명령이 있는 경우 사용한다.
    *   2. 매개변수 있는 생성자의 경우 매새변수로 전달받은 값으로 필드를 초기화하며
    *       인스턴스를 생성할 목적으로 주로 사용된다.
    *   3. 작성한 생성자 외에는 인스턴스를 생성하는 방법을 제공하지 않는다는 의미
    *       따라서, 인스턴스를 생성하는 방법을 제한하기 위한 용도로 사용할 수 있다. (초기값 정달 강제)
    *  */
    ...
    *   매개변수가 있는 생성자가 한 개라도 존재하는 경우에는 기본 생성자를 자동으로 추가해주기 않기 때문에
    *   기본 생성자가 필요한 경우에는 반드시 명시적으로 작성해줘야 한다.
    *  */
    ...
    public User(String id, String pwd, String name){
        ...
        /* 필기. 매개변수가 있는 생성자의 주 목적은 인스턴스 생성 시점에 매개변수로 전달
...
//        this.enrollDate = enrollDate;
        ...
        /* 목차. 3-2. 사정에 작성되어 있는 다른 생성자 함수를 이용하여 초기화  <- 잘 안 쓰임 */
        /* 필기.
        *   this() 사용하기
        ...
        *  */
        this(id, pwd, name); // 미리 작성한 세 개의 필드를 초기화 하는 생성자로 매개젼수로 받은 값 전달.
        this.enrollDate = enrollDate;
    ...
    public String getInfo(){
        ...
        return "User [id = " + this.id + ", psw = " + this.pwd + ", name = " + this.name + ", enrollDate = " + this.enrollDate + "]";
    }
