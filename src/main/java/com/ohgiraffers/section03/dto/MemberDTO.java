...
public class MemberDTO {
    ...
    *  */
    ...
    /* 필기.
    *   설정자(getter) / 접근자(getter) 의 경우 실무에서 암묵적으로 통용되는 작성 규칙이 존재한다.
    *   설정자(getter) 작성규칙
    *   : 필드값을 변경할 목적의 매개 변수를 변경하려는 필드와 같은 자료형으로 선언하고,
    *   호출 당시 전달되는 매개변수의 값을 이용하여 필드의 값을 변경한다.
    *   [표현식]
    *   public void ser 필드명 (매개변수){
    *       필드 = 매개변수;
    *   }
    *   [작성예시]
    ...
    *   }
    *   접근자(getter) 작성규칙
    *   : 필드이 값을 반황받는 목적의 메소드 집합을 의미한다.
    *   각 접근자는 하나의 필드에만 접근하도록 한다.
    *   필드에 접근해서 기록된 값을 return 을 이용하여 반환하며, 이 떄 반환 타입은 값의 자료형과 일치.
    *   [표현식]
    *   public 반환형 get 필드명(){
    *       return 반환값;
    *   }
